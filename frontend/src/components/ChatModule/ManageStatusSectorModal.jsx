...
import { X, Trash2, Plus, Pencil, Check, Settings2, ShieldCheck } from "lucide-react";
...
const ManageStatusSectorModal = ({ isOpen, onClose, initialType = "status", allStatusOptions, sectors, onAdd, onDelete, onUpdate }) => {
  ...
  const handleEditSave = (id) => {
    onUpdate(activeTab, id, { name: editName, color: editColor });
    setEditingId(null);
  };
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(11, 27, 33, 0.75)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 2000, backdropFilter: "blur(8px)" }}>
      <div style={{ background: "white", borderRadius: "24px", width: "500px", maxWidth: "90%", maxHeight: "85vh", display: "flex", flexDirection: "column", overflow: "hidden", boxShadow: "0 25px 60px rgba(0,0,0,0.3)" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
            {list.map(item => (
              <div key={item._id || item.name} style={{ 
                padding: "12px 16px", borderRadius: "16px", border: "1px solid #f1f5f9", 
                ...
                {editingId === (item._id || item.name) ? (
                  <div style={{ flex: 1, display: "flex", gap: "10px", alignItems: "center" }}>
                    ...
                    <button onClick={() => handleEditSave(item._id || item.name)} style={{ background: "#00a884", color: "white", border: "none", padding: "6px", borderRadius: "8px", cursor: "pointer" }}>
                      <Check size={16} />
                    </button>
                  ...
                  <>
                    ...
                    <div style={{ display: "flex", gap: "8px" }}>
                      ...
                      <button onClick={() => onDelete(activeTab, item._id || item.name)} style={{ padding: "6px", background: "none", border: "none", color: "#f87171", cursor: "pointer" }}>
                        <Trash2 size={14} />
                      </button>
                    </div>
                  </>
